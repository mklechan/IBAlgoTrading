...
class ArtemisIBWrapper(EWrapper):
    ...
    def historicalData(self, reqId, date, open, high,
        ...
        if date[:8] == 'finished':
	    self.histClose = self.lastClose
            print "History request complete"
        else:
	    try:
                date = datetime.strptime(date, "%Y%m%d").strftime("%d %b %Y")
	    except:
                date = datetime.strptime(date, "%Y%m%d  %H:%M:%S").strftime("%d %b %Y %H:%M:%S")
	...
	if key == "BuyingPower":
	   ...
	   self.totalCashBalance = value
	elif key == "NetLiquidation":
	   self.netLiquidation = value
	...
	print "Message ID:", str(msgId), " Message type:", str(msgType), " Message: ", message, " Originating Exchange:", origExchange
	pass
    ...
    def commissionReport(self, commission_rpt):
        print "Commision from trade: ", commission_rpt.commission
